...
int authorisation_check(const char *username, const char *password, const char *serverIP, int pop3Port, int *clientSkt);
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        case 1:
            ...
            {
                int clientSocket;
                int proceed = authorisation_check(username, password, serverIP, pop3Port, &clientSocket);
                if (!proceed)
                {
                    printf("Authorisation failed! Incorrect username or password.\nServer closed the connection\n");
                    close(clientSocket);
                }
                else
                {
...
int send_response(const char *message, int clientSocket)
{
    char temp_buff[100];
...
int authorisation_check(const char *username, const char *password, const char *serverIP, int pop3Port, int *clientSkt)
{
    ...
    *clientSkt = clientSocket;
    ...
    send_response(command, clientSocket);
    memset(command, 0, sizeof(command));

    // Read the response to USER command
    memset(full_buff, 0, sizeof(full_buff));
    read_response(clientSocket, full_buff);
    if ((strncmp(full_buff, "+OK ", 4) != 0) || (strncmp(full_buff, "-ERR ", 5) != 0))
    {
        fprintf(stderr, "Error: Unexpected response from server: %s\n", full_buff);
        return 0;
    }

    if (strncmp(full_buff, "-ERR ", 5) == 0)
    {
        printf("Authorisation failed! Incorrect username\n");
        return 0;
    }
    ...
    if (strncmp(full_buff, "+OK ", 4) == 0)
    {
        ...
        // Read the response to PASS command
        memset(full_buff, 0, sizeof(full_buff));
        read_response(clientSocket, full_buff);
        if ((strncmp(full_buff, "+OK ", 4) != 0) || (strncmp(full_buff, "-ERR ", 5) != 0))
        {
        ...
        if (strncmp(full_buff, "-ERR ", 5) == 0)
        {
            printf("Authorisation failed! Incorrect password\n");
            return 0;
        }
